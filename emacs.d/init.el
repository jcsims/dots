...
(use-package bug-reference
  :ensure f
  :config
  (setq bug-reference-bug-regexp "\\(\\(RATE-[0-9]+\\|LOS-[0-9]+\\|AR-[0-9]+\\|RENG-[0-9]+\\)\\)"
...
(use-package eglot
  ...
  :custom (eglot-connect-timeout 360))
...
;; Borrowed mostly from the combobulate README at https://github.com/mickeynp/combobulate
(use-package treesit
  :ensure f
  :mode (("\\.tsx\\'" . tsx-ts-mode))
  :preface
  (defun jcs/setup-install-grammars ()
...
format. With PREFIX, copy to kill ring."
  ...
  ;; Bookmark helper
  (load-file-if-present "work-links/work-bookmarks.el")
